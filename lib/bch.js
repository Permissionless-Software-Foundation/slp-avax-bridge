...
class BCHLib {
  ...
  /**
   * @api bchAvaxBridge.bchjs.mintSlp() mintSlp()
   * @apiName mintSlp
   * @apiGroup BCH
   * @apiDescription Mint tokens on the BCH chain.
   *
   ...
   * try {
   *  const num = 10
   *  const txid = await bchAvaxBridge.bchjs.mintSlp(num)
   ...
   */
  async mintSlp (num, receiverAddress) {
    try {
      ...
      const receiver = _this.getValidAddress(receiverAddress, legacyAddress)
  ...
  /**
   * @api bchAvaxBridge.bchjs.burnSlp() burnSlp()
   * @apiName burnSlp
   * @apiGroup BCH
   * @apiDescription Burn tokens on the BCH chain.
   *
   * This method expects several environment variables to be set before running.
   * It will throw an error if these environment variables are not set.
   *
   ...
   * try {
   *  const num = 1
   *  const txid = await bchAvaxBridge.bchjs.burnSlp(num)
   *  console.log(`txid: ${txid}`)
   * } catch (error) {
   *  console.error(error)
   * }
  ...
  getValidAddress (receiver, fallback) {
    try {
      ...
      return _this.bchjs.Address.toLegacyAddress(receiver)
    } catch (error) {
